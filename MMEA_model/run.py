...
def save_model(model, optimizer, save_variable_list, args): # 保留模型配置参数、模型参数和优化器、实体嵌入和关系嵌入
    '''
...
def main(args):
    if (not args.do_train) and (not args.do_valid) and (not args.do_test): 
        raise ValueError('one of train/val/test mode must be choosed.') # raise ValueError 
    ...
    with open(os.path.join(args.data_path, 'entities.dict')) as fin:   # 将实体和关系映射到整数ID
        entity2id = dict()
        for line in fin:
            eid, entity = line.strip().split('\t')  # '0	/m/014_24'
            entity2id[entity] = int(eid)  # {'/m/014_24': 0}
    ...
    args.nrelation = nrelation # rgs.nrelation
...
# CUDA_VISIBLE_DEVICES=0 python -u codes/run.py --do_train  --cuda  --do_valid  --do_test  --data_path ./dataset/FB15k-237/RotatE_FB15k-237 --test_batch_size 16 -de